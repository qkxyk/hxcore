...
namespace HXCloud.Service
{
    public class UserProfile : Profile
    {
        public UserProfile()
        {
            ...
            CreateMap<UserModel, UserData>().ForMember(dest => dest.Create, opt => opt.MapFrom(src => src.CreateTime)).ForMember(d => d.Status, s => s.MapFrom(a => (int)a.Status))
                .ForMember(d => d.LastLogin, s => s.MapFrom(a => a.LastLogin == null ? null : a.LastLogin)).ForMember(dest=>dest.UserType,opt=>opt.MapFrom(
                    src=>(UserType)src.UserType));
            CreateMap<UserModel, UserInfoDto>().ForMember(dest=>dest.Create,opt=>opt.MapFrom(src=>src.CreateTime)).ForMember(dest => dest.GroupId,
