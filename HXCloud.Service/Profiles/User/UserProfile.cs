...
namespace HXCloud.Service
{
    public class UserProfile : Profile
    {
        public UserProfile()
        {
            ...
            CreateMap<UserModel, UserData>().ForMember(d => d.Create, s => s.MapFrom(b => b.CreateTime)).ForMember(
                d => d.Status, s => s.MapFrom(a => (int)a.Status)).ForMember(d => d.LastLogin, s => s.MapFrom(a => a.LastLogin == null ? null : a.LastLogin));
            CreateMap<UserModel, UserInfoDto>().ForMember(dest => dest.GroupId, opt => opt.MapFrom(src => src.Group.Id)).ForMember(dest => dest.GroupName,
